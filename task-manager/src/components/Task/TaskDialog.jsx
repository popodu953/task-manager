...
const TaskDialog = ({ task, refetch }) => {
  ...
  const [trashTask, { isLoading: isDeleting }] = useTrashTaskMutation();
  ...
  const duplicateHandler = async () => {
    try {
      ...
      if (result.data.status) {
        ...
        toast.error(result.data.message || "Failed to duplicate task");
      }
    } catch (error) {
  ...
  const deleteHandler = async () => {
    try {
      const result = await trashTask({ id: task._id });
      if (result.data.status) {
  ...
  return (
    <>
      ...
      <AddSubTask open={open} setOpen={setOpen} id={task._id} refetch={refetch} />
